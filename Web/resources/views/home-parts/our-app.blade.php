<article class="our-app" id="our-app">
    <h2 class="article-title">Nasza aplikacja</h2>
    <h4 class="article-subtitle">Ułatw sobie życie, korzystając z niej</h4>
    <section class="mockups">
        <img src="{{ asset('resources/img/mockups/home.png') }}" alt="">
        <div class="download-app">
            <img class="download" src="{{ asset('resources/img/mockups/download-app.png') }}" alt="">
            <button class="integrated">Pobierz aplikacje</button>
        </div>
        <img src="{{ asset('resources/img/mockups/dashboard.png') }}" alt="">
    </section>
</article>